(() => {
  ...
  function messageStatus(message) {
    if (message.seen_at) {
      ...
      const seenLabel = seenDate ? `Seen at ${seenDate.toLocaleString()}` : "Seen";
    ...
    if (message.delivered_at) {
      ...
      const deliveredLabel = deliveredDate ? `Delivered at ${deliveredDate.toLocaleString()}` : "Delivered";
  ...
  function renderMessages(keepPosition = false) {
    ...
    state.chatMessages.forEach((message) => {
      ...
      if (mine) { wrap.append(actions, bubble); } else { wrap.append(bubble, actions); }
  ...
  function renderPosts() {
    ...
    host.innerHTML = state.posts.map((post) => `
      <article class="postCard" data-post-id="${post.id}">
        <header class="postHeader">${avatarHtml(post, 42, state.users.find((u) => u.id === post.user_id)?.online || post.user_id === state.me.id)}<div class="postWho"><b>${escapeHtml(post.display_name || post.username)}</b><small>@${escapeHtml(post.username)} • ${relativeTime(post.created_at)}</small></div>${post.user_id === state.me.id ? '<button class="postMenu" data-post-action="delete" title="Delete post" type="button">⋮</button>' : ""}</header>
        ${post.text ? `<p class="postText">${escapeHtml(post.text)}</p>` : ""}
        ${postMediaHtml(post)}
        <footer class="postActions"><button class="${post.liked_by_me ? "liked" : ""}" data-post-action="like" type="button">${post.liked_by_me ? "♥" : "♡"} <span>${post.like_count || 0}</span></button><button data-post-action="comments" type="button">◌ ${post.comment_count || 0} comments</button><button class="sharePostBtn" data-post-action="share" type="button">↗ Share</button></footer>
      </article>`).join("");
  }
  ...
  function renderHomeStories() {
    ...
    host.innerHTML = `<button class="storyBubble addStory" data-create="story" type="button"><span class="storyRing">${avatarHtml(state.me, 56)}</span><small>Your story</small></button>` + unique.map((story) => {
      ...
      return `<button class="storyBubble" data-story-id="${story.id}" type="button"><span class="storyRing${story.viewed_by_me ? " viewed" : ""}">${thumb}</span><small>${story.user_id === state.me.id ? "Your story" : escapeHtml(story.display_name || story.username)}</small></button>`;
    }).join("");
  }
  ...
  function showCurrentStory() {
    ...
    $("storyViewerTime").textContent = `${relativeTime(story.created_at)}${story.user_id === state.me.id ? ` • ${story.view_count || 0} views` : ""}`;
    ...
    if (story.user_id !== state.me.id && !story.viewed_by_me) {
  ...
  $("videoCallBtn").addEventListener("click", () => toast("Video calling is planned for the next phase."));
  ...
  document.querySelectorAll("[data-ai-prompt]").forEach((button) => button.addEventListener("click", () => { $("aiInput").value = button.dataset.aiPrompt; $("aiInput").focus(); }));

  $("editProfileBtn").addEventListener("click", () => {
    if ($("editProfileBtn").dataset.profileAction === "edit") openProfileEditor();
    else {
      const user = state.users.find((item) => item.id === state.profileViewing);
