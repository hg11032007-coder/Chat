(() => {
  ...
  function connectSocket() {
    ...
    state.socket.on("presence", ({ user_id: userId, online, last_seen: lastSeen }) => {
      ...
      if (state.profile?.user && sameId(state.profile.user.id, userId)) {
        state.profile.user.online = online;
        state.profile.user.last_seen = lastSeen;
        renderProfile();
      }
      renderChatList();
      renderDiscover();
    ...
    state.socket.on("admin_access_changed", (update = {}) => {
      if (!state.me) return;
      state.me.is_admin = Boolean(update.is_admin);
      state.me.admin_role = update.admin_role || "user";
      state.me.admin_permissions = Array.isArray(update.admin_permissions) ? update.admin_permissions : [];
      localStorage.setItem("sandeep_me", JSON.stringify(state.me));
      if (state.profile?.user && sameId(state.profile.user.id, state.me.id)) {
        state.profile.user.is_admin = state.me.is_admin;
        state.profile.user.admin_role = state.me.admin_role;
        state.profile.user.admin_permissions = state.me.admin_permissions;
        renderProfile();
      }
      renderIdentity();
      toast(state.me.is_admin ? "Admin access granted. Open Admin panel to continue." : "Your admin access was removed.", state.me.is_admin ? "success" : "");
    });
  ...
  async function publishPost() {
    ...
    try {
      ...
      state.posts = [post, ...state.posts.filter((item) => !sameId(item.id, post.id))];
      renderPosts();
      closeModal("postModal");
      clearPostPreview();
      // Reload from SQLite instead of relying only on temporary browser state.
      await loadPosts();
      toast("Post saved to your account", "success");
  ...
  async function sendAiMessage(event) {
    ...
    try {
      ...
      typing.remove();
      host.insertAdjacentHTML("beforeend", aiMessageHtml("Sorry yaar, Nova abhi connect nahi ho pa rahi. Thodi der baad phir try karna—main yahin hoon. 🙂", false));
    } finally {
      state.aiBusy = false;
      host.scrollTop = host.scrollHeight;
    }
  ...
  function renderProfile() {
    ...
    const role = user.admin_role === "owner" ? "owner" : (user.admin_role === "admin" || user.is_admin ? "admin" : "user");
    const profileColumn = $("profileColumn");
    profileColumn.classList.remove("role-owner", "role-admin", "role-user");
    profileColumn.classList.add(`role-${role}`);

    const roleBadge = $("profileRoleBadge");
    const roleStatement = $("profileRoleStatement");
    if (role === "owner") {
      roleBadge.className = "profileRoleBadge owner";
      roleBadge.innerHTML = '<i>♛</i><b>OWNER</b><em>✓ VERIFIED</em>';
      roleStatement.className = "profileRoleStatement owner";
      roleStatement.innerHTML = '<span class="roleSeal">♛</span><span><b>Official owner of Sandeep Chat</b><small>Founder • Protected platform account</small></span><i class="verifiedSeal">✓ VERIFIED</i>';
    } else if (role === "admin") {
      roleBadge.className = "profileRoleBadge admin";
      roleBadge.innerHTML = '<i>◆</i><b>ADMIN</b>';
      roleStatement.className = "profileRoleStatement admin";
      roleStatement.innerHTML = '<span class="roleSeal">◆</span><span><b>Sandeep Chat administrator</b><small>Delegated access account</small></span>';
    } else {
      roleBadge.className = "profileRoleBadge hidden";
      roleBadge.textContent = "";
      roleStatement.className = "profileRoleStatement hidden";
      roleStatement.textContent = "";
    }
    ...
    presence.classList.toggle("offline", !user.online);
    presence.textContent = user.online ? "● Online" : user.last_seen ? `● Offline • Last online ${relativeTime(user.last_seen)}` : "● Offline";
  ...
  async function uploadAvatar(file) {
    ...
    try {
      ...
      // Read the account back from SQLite so the UI never shows a browser-only photo.
      const savedUser = await api("/api/me");
      if (!savedUser.avatar || savedUser.avatar !== data.avatar) throw new Error("Profile photo could not be confirmed");
      state.me = savedUser;
      localStorage.setItem("sandeep_me", JSON.stringify(state.me));
      renderIdentity();
      openProfileEditor();
      await Promise.all([loadUsers(), loadProfile(state.me.id)]);
      toast("Profile photo saved to your account", "success");
    } catch (error) { toast(error.message, "error"); }
  }
