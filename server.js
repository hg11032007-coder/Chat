...
app.post("/api/login", authRateLimit, async (req, res) => {
  const username = String(req.body.username || "").trim();
  // Legacy databases may contain usernames that differ only by letter case.
  // Prefer the exact account the person typed, then fall back case-insensitively.
...
async function requestNovaReply(messages) {
  const errors = [];
  for (const model of novaModelCandidates()) {
    ...
    try {
      ...
      if (looksLikeSafetyDiagnostic(reply)) {
        errors.push(`${model}: classifier output rejected`);
        continue;
      }
      return reply;
    } catch (error) {
      errors.push(`${model}: ${error.name === "AbortError" ? "request timed out" : error.message}`);
    } finally {
      clearTimeout(timer);
    }
  }
