...
const ADMIN_USERNAME = String(process.env.ADMIN_USERNAME || "sandeep").trim().toLowerCase();
...
(function bootstrapOwner() {
  ...
  // One-time migration from the old username-based admin system. Prefer the
  // earliest legacy admin matching ADMIN_USERNAME, then any existing legacy admin.
  let candidate = db.prepare(`SELECT * FROM users
    WHERE is_admin=1 AND LOWER(username)=?
    ORDER BY id LIMIT 1`).get(ADMIN_USERNAME);
  if (!candidate) candidate = db.prepare("SELECT * FROM users WHERE is_admin=1 ORDER BY id LIMIT 1").get();
  if (!candidate && ADMIN_PASSWORD) {
    candidate = db.prepare("SELECT * FROM users WHERE LOWER(username)=? ORDER BY id LIMIT 1").get(ADMIN_USERNAME);
  }
  ...
  db.transaction(() => {
    ...
    if (ADMIN_PASSWORD) {
      ...
      const info = db.prepare(`INSERT INTO users
        ...
        VALUES(?,?,?,1,'owner','[]')`)
        .run(ADMIN_USERNAME, bcrypt.hashSync(ADMIN_PASSWORD, 11), ADMIN_USERNAME);
      console.log(`Owner account created: @${ADMIN_USERNAME} (user ${info.lastInsertRowid})`);
    } else {
