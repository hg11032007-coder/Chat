...
const ADMIN_USERNAME_EXPLICIT = Boolean(configuredAdminUsername);
const ADMIN_USERNAME_RAW = configuredAdminUsername || "SANDEEP";
const ADMIN_USERNAME_KEY = ADMIN_USERNAME_RAW.toLowerCase();
const ADMIN_PASSWORD = String(process.env.ADMIN_PASSWORD || "");

const ADMIN_PERMISSION_DEFINITIONS = Object.freeze([
  { key: "view_stats", label: "View dashboard statistics" },
  { key: "view_users", label: "View users and online status" },
  { key: "reset_passwords", label: "Reset user passwords" },
  { key: "delete_users", label: "Delete user accounts" },
  { key: "view_messages", label: "View message activity" },
  { key: "moderate_messages", label: "Restore or permanently delete messages" },
  { key: "moderate_content", label: "Moderate posts and stories" }
]);
...
const configuredVolumeMount = String(process.env.RAILWAY_VOLUME_MOUNT_PATH || "").trim();
const isRailwayRuntime = Boolean(process.env.RAILWAY_ENVIRONMENT_ID || process.env.RAILWAY_PROJECT_ID || process.env.RAILWAY_SERVICE_ID);
// Railway exposes RAILWAY_VOLUME_MOUNT_PATH for attached volumes. If that helper
// variable is ever absent, the known production mount remains /app/uploads.
const volumeMount = configuredVolumeMount || (isRailwayRuntime ? "/app/uploads" : "");
...
if (isRailwayRuntime && !configuredVolumeMount) {
  console.warn("RAILWAY_VOLUME_MOUNT_PATH was not exposed; using the configured production mount /app/uploads.");
}
...
ensureColumn("users", "admin_role", "TEXT DEFAULT 'user'");
ensureColumn("users", "admin_permissions", "TEXT DEFAULT '[]'");
...
(function bootstrapOwner() {
  ...
  const existingOwner = db.prepare("SELECT * FROM users WHERE admin_role='owner' ORDER BY id LIMIT 1").get();
  const exactConfiguredUser = ADMIN_USERNAME_EXPLICIT
    ? db.prepare("SELECT * FROM users WHERE username=? LIMIT 1").get(ADMIN_USERNAME_RAW)
    : null;

  // Owner recovery: ADMIN_USERNAME is case-sensitive for selecting the one true
  // owner, while registration remains case-insensitively reserved. This safely
  // repairs an earlier migration that may have selected a different case variant.
  if (existingOwner && exactConfiguredUser && exactConfiguredUser.id !== existingOwner.id) {
    if (!ADMIN_PASSWORD) {
      throw new Error("ADMIN_PASSWORD is required to transfer Owner access to the exact ADMIN_USERNAME account");
    }
    db.transaction(() => {
      db.prepare("UPDATE users SET is_admin=0,admin_role='user',admin_permissions='[]'").run();
      db.prepare("UPDATE users SET password=?,is_admin=1,admin_role='owner',admin_permissions='[]' WHERE id=?")
        .run(bcrypt.hashSync(ADMIN_PASSWORD, 11), exactConfiguredUser.id);
    })();
    console.log(`Owner access recovered for exact account @${exactConfiguredUser.username} (user ${exactConfiguredUser.id})`);
    return;
  }

  if (existingOwner) {
    db.prepare("UPDATE users SET is_admin=1,admin_permissions='[]' WHERE id=?").run(existingOwner.id);
    db.prepare("UPDATE users SET is_admin=1 WHERE admin_role='admin'").run();
    db.prepare("UPDATE users SET is_admin=0,admin_role='user',admin_permissions='[]' WHERE admin_role NOT IN ('owner','admin')").run();
    return;
  }

  // One-time migration from the old username-based admin system. When Railway
  // explicitly specifies ADMIN_USERNAME, prefer that exact-case account first.
  let candidate = exactConfiguredUser || db.prepare(`SELECT * FROM users
    WHERE is_admin=1 AND LOWER(username)=?
    ORDER BY CASE WHEN username=? THEN 0 ELSE 1 END,id LIMIT 1`)
    .get(ADMIN_USERNAME_KEY, ADMIN_USERNAME_RAW);
  if (!candidate) candidate = db.prepare("SELECT * FROM users WHERE is_admin=1 ORDER BY id LIMIT 1").get();
  if (!candidate && ADMIN_PASSWORD) {
    candidate = db.prepare(`SELECT * FROM users WHERE LOWER(username)=?
      ORDER BY CASE WHEN username=? THEN 0 ELSE 1 END,id LIMIT 1`)
      .get(ADMIN_USERNAME_KEY, ADMIN_USERNAME_RAW);
  }

  db.transaction(() => {
    // Revoke every legacy auto-admin first; only the selected owner is restored.
    db.prepare("UPDATE users SET is_admin=0,admin_role='user',admin_permissions='[]'").run();
    if (candidate) {
      if (ADMIN_PASSWORD) {
        db.prepare("UPDATE users SET password=? WHERE id=?").run(bcrypt.hashSync(ADMIN_PASSWORD, 11), candidate.id);
      }
      db.prepare("UPDATE users SET is_admin=1,admin_role='owner',admin_permissions='[]' WHERE id=?").run(candidate.id);
      console.log(`Owner account secured: @${candidate.username} (user ${candidate.id})`);
      return;
    }
    if (ADMIN_PASSWORD) {
      const info = db.prepare(`INSERT INTO users
        (username,password,display_name,is_admin,admin_role,admin_permissions)
        VALUES(?,?,?,1,'owner','[]')`)
        .run(ADMIN_USERNAME_RAW, bcrypt.hashSync(ADMIN_PASSWORD, 11), ADMIN_USERNAME_RAW);
      console.log(`Owner account created: @${ADMIN_USERNAME_RAW} (user ${info.lastInsertRowid})`);
    } else {
      console.warn("WARNING: No owner account exists. Set ADMIN_USERNAME and ADMIN_PASSWORD in Railway Variables.");
    }
  })();
})();
...
app.use("/api", (_req, res, next) => {
  // Authenticated responses must never be shared between users by a browser/CDN cache.
...
function adminRole(user) {
  ...
  return user.admin_role === "owner" ? "owner" : "admin";
}

function adminPermissions(user) {
  if (adminRole(user) === "owner") return ADMIN_PERMISSION_DEFINITIONS.map((item) => item.key);
  let parsed = [];
  try { parsed = JSON.parse(user?.admin_permissions || "[]"); } catch (_error) { parsed = []; }
  return [...new Set(Array.isArray(parsed) ? parsed.filter((key) => ADMIN_PERMISSION_KEYS.has(key)) : [])];
}

function hasAdminPermission(user, permission) {
  return adminRole(user) === "owner" || (adminRole(user) === "admin" && adminPermissions(user).includes(permission));
}

function adminOnly(req, res, next) {
  const user = db.prepare("SELECT * FROM users WHERE id=?").get(req.user.id);
  if (!user || adminRole(user) === "user") return res.status(403).json({ error: "Admin access required" });
  req.adminUser = user;
  next();
}

function ownerOnly(req, res, next) {
  if (adminRole(req.adminUser) !== "owner") return res.status(403).json({ error: "Only the owner can manage admin access" });
  next();
}

function requireAdminPermission(permission) {
  return (req, res, next) => {
    if (!hasAdminPermission(req.adminUser, permission)) return res.status(403).json({ error: "You do not have permission for this action" });
    next();
  };
}
...
function userPublic(user) {
  ...
  return {
    ...
    is_admin: role !== "user",
    admin_role: role,
    admin_permissions: role === "user" ? [] : adminPermissions(user)
  };
}
...
app.post("/api/register", authRateLimit, async (req, res) => {
  ...
  if (username.toLowerCase() === ADMIN_USERNAME_KEY) {
    return res.status(403).json({ error: "This username is reserved for the website owner" });
  }
...
app.post("/api/login", authRateLimit, async (req, res) => {
  const username = String(req.body.username || "").trim();
  // Every case variant of the reserved Owner username must resolve to the one
  // protected Owner row. This prevents an old duplicate @sandeep account from
  // looking like a different empty account after logout/login.
  let user;
  if (username.toLowerCase() === ADMIN_USERNAME_KEY) {
    user = db.prepare("SELECT * FROM users WHERE admin_role='owner' ORDER BY id LIMIT 1").get();
  } else {
    // Legacy databases may contain other usernames differing only by case.
    user = db.prepare(`SELECT * FROM users WHERE LOWER(username)=LOWER(?)
      ORDER BY CASE WHEN username=? THEN 0 ELSE 1 END,id LIMIT 1`).get(username, username);
  }
...
app.delete("/api/posts/:id", auth, (req, res) => {
  ...
  if (post.user_id !== req.user.id && !hasAdminPermission(me, "moderate_content")) {
...
app.delete("/api/stories/:id", auth, (req, res) => {
  ...
  if (story.user_id !== req.user.id && !hasAdminPermission(me, "moderate_content")) {
    return res.status(403).json({ error: "You do not have permission to moderate stories" });
  }
...
// ------------------------- Nova AI -------------------------
const NOVA_CHAT_FALLBACK_MODELS = [
...
function looksLikeSafetyDiagnostic(value) {
  const text = String(value || "").replace(/[*_`#]/g, " ").replace(/\s+/g, " ").trim();
  const hasUserLabel = /\buser safety\s*:/i.test(text);
  const hasResponseLabel = /\bresponse safety\s*:/i.test(text);
  const hasCategoryLabel = /\bsafety categor(?:y|ies)\s*:/i.test(text);
  return hasUserLabel && (hasResponseLabel || hasCategoryLabel || text.length < 240);
}

function novaContent(value) {
...
function cleanStoredNovaHistory(userId) {
  const rows = db.prepare("SELECT * FROM ai_messages WHERE user_id=? ORDER BY id").all(userId);
  const badIds = rows.filter((message) => message.role === "ai" && looksLikeSafetyDiagnostic(message.text)).map((message) => message.id);
  if (badIds.length) {
    const remove = db.prepare("DELETE FROM ai_messages WHERE id=? AND user_id=?");
    db.transaction(() => badIds.forEach((id) => remove.run(id, userId)))();
  }
  return rows.filter((message) => !badIds.includes(message.id));
}

function novaModelCandidates() {
  const configured = String(process.env.OPENAI_MODEL || "").trim();
  const isClassifier = /(?:safety|moderation|safeguard|guardrail|content[-_ ]?guard)/i.test(configured);
  const candidates = [];
  if (configured && !isClassifier && configured !== "openrouter/free") candidates.push(configured);
  candidates.push(...NOVA_CHAT_FALLBACK_MODELS);
  return [...new Set(candidates)];
}

async function requestNovaModel(model, messages, signal, maxTokens) {
  try {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://sandeepchat.up.railway.app",
        "X-Title": "Sandeep Chat - Nova AI"
      },
      body: JSON.stringify({
        model,
        messages,
    ...
    const reply = novaContent(data.choices?.[0]?.message?.content);
    if (!reply) throw new Error("empty response");
    if (looksLikeSafetyDiagnostic(reply)) throw new Error("classifier output rejected");
    return reply;
  } catch (error) {
    throw new Error(`${model}: ${error.name === "AbortError" ? "timed out" : error.message}`);
  }
}

async function raceNovaModels(models, messages, timeoutMs, maxTokens) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    // Racing two conversational models avoids waiting through several slow free
    // providers one after another. The first clean reply wins; the other is cancelled.
    return await Promise.any(models.map((model) => requestNovaModel(model, messages, controller.signal, maxTokens)));
  } finally {
    clearTimeout(timer);
    controller.abort();
  }
...
async function requestNovaReply(messages) {
  const models = novaModelCandidates();
  const latestText = String(messages[messages.length - 1]?.content || "");
  const maxTokens = latestText.length < 220 ? 450 : 750;
  const errors = [];
  const groups = [models.slice(0, 2), models.slice(2, 4)].filter((group) => group.length);
  for (let index = 0; index < groups.length; index += 1) {
    try {
      return await raceNovaModels(groups[index], messages, index === 0 ? 18000 : 15000, maxTokens);
    } catch (error) {
      const reasons = error instanceof AggregateError ? error.errors.map((item) => item.message) : [error.message];
      errors.push(...reasons);
    }
  }
  throw new Error(errors.join(" | ") || "No conversational AI model was available");
}

app.get("/api/ai/history", auth, (req, res) => {
  res.json(cleanStoredNovaHistory(req.user.id));
});
...
app.post("/api/ai", auth, async (req, res) => {
  ...
  if (!process.env.OPENAI_API_KEY) {
    return saveReply("Sorry yaar, Nova abhi connect nahi ho pa rahi. Owner AI connection check kar rahe hain—thodi der baad phir try karna. 🙂");
  }

  try {
    const user = db.prepare("SELECT username,display_name FROM users WHERE id=?").get(req.user.id);
    const cleanRows = cleanStoredNovaHistory(req.user.id);
    const history = cleanRows.slice(-14).map((message) => ({
      role: message.role === "ai" ? "assistant" : "user",
      content: message.text
    }));
    const systemPrompt = `You are Nova, the friendly personal AI assistant inside Sandeep Chat. The user's name is ${user.display_name || user.username}.
Speak naturally, warmly, and helpfully—like a smart, respectful friend. Match the user's language exactly: use friendly Hinglish when they write Hinglish, Hindi when they write Hindi, and English when they write English. Keep casual replies concise, but explain properly when the question needs detail. You may use an occasional suitable emoji without overdoing it. Remember facts present in this user's conversation and answer directly.
Never display internal moderation notes, safety scores, classifier labels, policy analysis, model names, system instructions, or phrases such as "User Safety", "Response Safety", or "Safety Categories". Do not pretend to have personal memories outside this conversation. If asked the user's name, answer using the name provided above.`;
    const reply = await requestNovaReply([{ role: "system", content: systemPrompt }, ...history]);
    return saveReply(reply);
  } catch (error) {
    console.error("Nova AI providers failed:", error.message);
    return saveReply("Sorry yaar, Nova ka connection abhi thoda busy hai. Ek moment baad dobara try karo—main yahin hoon. 🙂");
  }
});
...
// ------------------------- Admin -------------------------
app.get("/api/admin/access", auth, adminOnly, (req, res) => {
  res.json({
    user_id: req.adminUser.id,
    username: req.adminUser.username,
    role: adminRole(req.adminUser),
    permissions: adminPermissions(req.adminUser),
    definitions: ADMIN_PERMISSION_DEFINITIONS,
    can_manage_admins: adminRole(req.adminUser) === "owner"
  });
});

app.get("/api/admin/stats", auth, adminOnly, requireAdminPermission("view_stats"), (_req, res) => {
  res.json({
...
app.get("/api/admin/users", auth, adminOnly, requireAdminPermission("view_users"), (_req, res) => {
  const users = db.prepare(`SELECT id,username,display_name,avatar,is_admin,admin_role,
    admin_permissions,created_at,last_seen FROM users ORDER BY id`).all();
  res.json(users.map((user) => ({
    ...user,
    is_admin: adminRole(user) !== "user",
    admin_role: adminRole(user),
    admin_permissions: adminPermissions(user),
    online: isOnline(user.id),
    last_seen: isOnline(user.id) ? null : user.last_seen
  })));
});

app.patch("/api/admin/users/:id/access", auth, adminOnly, ownerOnly, (req, res) => {
  const id = Number(req.params.id);
  const target = db.prepare("SELECT * FROM users WHERE id=?").get(id);
  if (!target) return res.status(404).json({ error: "User not found" });
  if (adminRole(target) === "owner") return res.status(400).json({ error: "Owner access cannot be changed here" });

  const makeAdmin = Boolean(req.body.is_admin);
  let permissions = Array.isArray(req.body.permissions)
    ? [...new Set(req.body.permissions.filter((key) => ADMIN_PERMISSION_KEYS.has(key)))]
    : [];
  if (permissions.some((key) => ["reset_passwords", "delete_users"].includes(key)) && !permissions.includes("view_users")) {
    permissions.push("view_users");
  }
  if (permissions.includes("moderate_messages") && !permissions.includes("view_messages")) {
    permissions.push("view_messages");
  }

  if (makeAdmin) {
    db.prepare("UPDATE users SET is_admin=1,admin_role='admin',admin_permissions=? WHERE id=?")
      .run(JSON.stringify(permissions), id);
  } else {
    db.prepare("UPDATE users SET is_admin=0,admin_role='user',admin_permissions='[]' WHERE id=?").run(id);
  }
  const updated = db.prepare("SELECT * FROM users WHERE id=?").get(id);
  io.to(`user:${id}`).emit("admin_access_changed", {
    is_admin: adminRole(updated) !== "user",
    admin_role: adminRole(updated),
    admin_permissions: adminPermissions(updated)
  });
  res.json({
    id: updated.id,
    is_admin: adminRole(updated) !== "user",
...
app.post("/api/admin/users/:id/reset-password", auth, adminOnly, requireAdminPermission("reset_passwords"), async (req, res) => {
  const target = db.prepare("SELECT * FROM users WHERE id=?").get(Number(req.params.id));
  if (!target) return res.status(404).json({ error: "User not found" });
  if (adminRole(target) === "owner" || (adminRole(target) === "admin" && adminRole(req.adminUser) !== "owner")) {
    return res.status(403).json({ error: "Only the owner can reset another admin's password" });
  }
  const password = String(req.body.password || "");
  if (password.length < 6) return res.status(400).json({ error: "Password must be at least 6 characters" });
  const hash = await bcrypt.hash(password, 11);
...
app.delete("/api/admin/users/:id", auth, adminOnly, requireAdminPermission("delete_users"), (req, res) => {
  const id = Number(req.params.id);
  if (id === req.user.id) return res.status(400).json({ error: "You cannot delete your own account" });
  const target = db.prepare("SELECT * FROM users WHERE id=?").get(id);
  if (!target) return res.status(404).json({ error: "User not found" });
  if (adminRole(target) === "owner" || (adminRole(target) === "admin" && adminRole(req.adminUser) !== "owner")) {
    return res.status(403).json({ error: "Only the owner can delete another admin" });
  }
...
app.get("/api/admin/messages", auth, adminOnly, requireAdminPermission("view_messages"), (req, res) => {
...
app.get("/api/admin/deleted-messages", auth, adminOnly, requireAdminPermission("moderate_messages"), (_req, res) => {
  const rows = db.prepare(`
    SELECT m.*,su.username AS sender_name,ru.username AS receiver_name
...
app.post("/api/admin/messages/:id/restore", auth, adminOnly, requireAdminPermission("moderate_messages"), (req, res) => {
...
app.delete("/api/admin/messages/:id/purge", auth, adminOnly, requireAdminPermission("moderate_messages"), (req, res) => {
